    ...
    <div className="flex flex-col w-full bg-zinc-950/20 backdrop-blur-xl p-8 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.4)] border border-white/10">
      <div className="mb-6 text-center sm:text-left">
        ...
        <p className="text-zinc-400 mt-1.5 text-sm">
          Join Annotator to manage your projects and taskers.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2 mt-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className={`flex items-center justify-center py-2.5 px-3 rounded-xl border cursor-pointer transition-all ${role === 'client' ? 'bg-indigo-600/20 border-indigo-500 text-white' : 'bg-zinc-900/50 border-white/10 text-zinc-400 hover:bg-zinc-900/80 hover:text-zinc-300'}`}>
              <input type="radio" name="role" value="client" checked={role === 'client'} onChange={() => setRole('client')} className="sr-only" />
              <span className="text-sm font-medium">Client</span>
            </label>
            <label className={`flex items-center justify-center py-2.5 px-3 rounded-xl border cursor-pointer transition-all ${role === 'tasker' ? 'bg-indigo-600/20 border-indigo-500 text-white' : 'bg-zinc-900/50 border-white/10 text-zinc-400 hover:bg-zinc-900/80 hover:text-zinc-300'}`}>
              <input type="radio" name="role" value="tasker" checked={role === 'tasker'} onChange={() => setRole('tasker')} className="sr-only" />
