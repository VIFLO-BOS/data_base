...
function TimeCell({
  ...
  return (
    ...
    >
      <span
        className={`text-sm leading-6 ${isFilled ? 'text-stone-900 font-medium' : 'text-stone-900 font-medium opacity-50'}`}
      >
        {isFilled ? value : '--'}
      </span>
      {!isFilled && (
...
export function TimesheetTable({
  ...
  return (
    <div className="w-full flex flex-col gap-4">
      ...
      <div className="w-full overflow-x-auto overflow-y-visible -mx-1 px-1 h-full">
        <div className="min-w-[900px] flex flex-col">
          {/* Column Headers */}
          <div className="w-full py-3 border-0 border-b shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] grid gap-2 items-center" style={{ gridTemplateColumns: `1.5fr 1.5fr repeat(${dayColumns.length}, 1fr) 1fr 1fr 48px` }}>
          ...
          {rows.map((row, index) => (
            <div
              key={index}
              className="w-full py-3 border-0 border-b shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] grid gap-2 items-center hover:bg-zinc-50 transition-colors group"
              style={{ position: 'relative', zIndex: openMenuIndex === index ? 40 : 'auto', gridTemplateColumns: `1.5fr 1.5fr repeat(${dayColumns.length}, 1fr) 1fr 1fr 48px` }}
            >
              ...
              <div className="flex justify-end items-center" style={{ position: 'static' }}>
                <button
